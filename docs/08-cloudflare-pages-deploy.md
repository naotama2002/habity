...
| Build command | `pnpm install && pnpm intl:compile && pnpm exec expo export --platform web && bash scripts/fix-cloudflare-assets.sh && bash scripts/inject-pwa-meta.sh && cp -r public/* dist/` | |
...
bash scripts/inject-pwa-meta.sh
cp -r public/* dist/
