...
**テストは「ある」だけでなく、ロジックの正しさを実質的に検証すること。**

- クエリ結果の加工、フラグ判定、状態の導出など **UIに直接影響するロジック** は、境界条件を含むテストを必ず書く
- 「テストが書きにくい」場合は設計の問題として捉え、テスト可能な形に分離する
- 形式的に「テストファイルがある」だけでは不十分。追加したロジックが壊れたときにテストが落ちる状態にする
